...
const DataTable = <T,>(props: DatatableProps<T>) => {
    const [builtUrl, setBuiltUrl] = useState<URL>(
        new URL(`/api/${props.url}`, import.meta.env.VITE_API_URL),
    );
        ...
        return (
            <th key={index} className="p-2 whitespace-nowrap">
                <div className="font-semibold text-left">
                    {element.name}
                    {element.has_sort && (
                        <button onClick={() => sortBy(element.original_name)}>
                    ...
                    <div className="w-full mx-auto bg-white shadow-lg rounded-sm border border-gray-200">
                        <header className="px-5 py-4 border-b border-gray-100">
                                ...
                                {props?.table_actions?.map((el, index) => {
                                    return (
                                        <button
                                            key={index}
                                            onClick={() => el.fn && el.fn()}
                                            className="bg-green-500 rounded-full text-lg text-white px-3 py-1 "
                                        >
                                            {el.icon}
                                        </button>
                                    );
                            ...
                            {props?.has_search && (
                                <div className="pt-2">
                                    <input
                                        value={searchParams?.search_param}
                                        onChange={(e) => {
                                            setSearchParams(
                                                (c) =>
                                                    c && {
                                        ...
                                        type="text"
                                        className=" curos autofocus border mb-2 bg-gray-100 border-gray-300 h-10 px-5 pr-16 rounded-lg text-sm focus:outline-none"
                        ...
                        {isLoading && <p>{t("common.please_wait")}</p>}
                                                        ...
                                                        >
                                                            {props.fields.map(
                                                                (f, index) => {
                                                                    ...
                                                                    throw new Error(
                                                                        `Column [${f.original_name}] is not part of response.`,
                                                                    );
                                                                },
                                                            )}
