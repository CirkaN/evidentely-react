        ...
        <Dialog.Root open={props.isOpen} >
            <Dialog.Content style={{ maxWidth: 450 }}>
                ...
                <form onSubmit={handleSubmit}>
                    {props.image.note}
                    <div className="mb-4">
                        <label className="block text-gray-600 text-sm font-medium mb-2">{t('common.note')}</label>
                        <input type="text"
                            value={imageProperties?.note ?? ""}
                            onChange={(e) => { setImageProperties((c) => c && { ...c, note: e.target.value }) }}
                            name="note" 
                    ...
                    <Flex gap="3" mt="4" justify="end">
                        <Dialog.Close>
                            <Button onClick={() => { cleanFunction() }} variant="soft" color="gray">
                                {t('common.cancel')}
                            </Button>
                        </Dialog.Close>
