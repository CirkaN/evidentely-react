...
const ShowSaleModal = (props: showSaleModalProps) => {
    ...
    const { refetch } = useQuery({
        ...
        queryFn: () =>
            axios_instance()
                .get(`/sale/${props.saleId}/notes`)
                .then((r) => setSaleNotes(r.data)),
        enabled: true,
    });
    useQuery({
        queryKey: ["sale_data"],
    ...
    const fieldActions: Action<SalePayment>[] = [
        {
            ...
            fn: (salePayment: SalePayment) => {
                raiseDeleteAlert(salePayment.id);
            },
        },
    ];
                    ...
                    <div>
                        <div className="flex flex-col justify-evenly sm:flex-row text-center  ">
                            <div className="pr-5">
                                ...
                                <p>
                                    {t("sales.sold_to")}:{" "}
                                    <b>{sale?.sold_to_name}</b>
                                </p>
                                <p>
                                    {t("sales.date_of_sale")}:{" "}
                                    <b>{sale?.sold_time_human}</b>
                                </p>
                            ...
                            <div className="pr-5">
                                ...
                                <p>
                                    {t("sales.sold_for_amount")}:{" "}
                                    <span className="text-green-700">
                                        {formatCurrency(sale?.price ?? "")}
                                    </span>{" "}
                                </p>
                                <p>
                                    {t("sales.paid_to_date")}:
                                    {formatCurrency(sale?.total_paid ?? "")}{" "}
                                </p>
                                <p>
                                    {t("sales.left_for_pay")}:{" "}
                                    {formatCurrency(sale?.pending_amount ?? "")}
                                </p>
                            </div>
                            <div>
                                <p className="font-bold text-xl text-green-700">
                                    {t("sales.quick_statistic")}:
                                </p>
                                <p>
                                    {t("sales.estimated_profit")}:{" "}
                                    <span className="text-green-700">
                                        {formatCurrency(
                                            sale?.expected_profit ?? "",
                                        )}
                                    </span>{" "}
                                </p>
                                <p>
