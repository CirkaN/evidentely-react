...
const CreateSaleModal = (props: CreateSaleProps) => {
    ...
    const [productTransformedList, setProductTransformedList] =
        useState<TransformedDataForSelect[]>();
    const [employeeTrasnformedList, setEmployeeTransformedList] =
        useState<TransformedDataForSelect[]>();
        ...
        if (e) {
            if (itemList) {
                const item = itemList.find(
                    (item) => item.id == e.value.toString(),
                );
                setForm(
                    (c) => c && { ...c, price: item?.selling_price ?? "0" },
                );
            }
