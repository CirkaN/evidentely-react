...
const ChargeClientModal = (props: ChargeClientProps) => {
    ...
    const { isLoading } = useQuery({
        ...
        queryFn: () =>
            axios_instance()
                .get(`/appointments/${props.appointment_id}`)
                .then((r) => {
                    setChargeParams(
                        (c) => c && { ...c, paid_amount: r.data.paid_amount },
                    );
                    setChargeParams(
                        (c) =>
                            c && { ...c, amount_to_be_paid: r.data.due_amount },
                    );
                }),
    });
    ...
    const saveCharge = () => {
        axios_instance()
            .post(`/appointments/${props.appointment_id}/charge`, chargeParams)
            .then(() => {
