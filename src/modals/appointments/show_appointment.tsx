...
const ShowAppointmentModal = (props: ShowAppointmentModalProps) => {
    ...
    const [isChargeClientModalOpen, setIsChargeClientModalOpen] =
        useState(false);
    ...
    const fetchAppointment = () => {
        axios_instance()
            .get(`/appointments/${props.appointmentId}`)
            .then((response) => {
    ...
    const updateEvent = () => {
        axios_instance()
            .put(`/appointments/${props.appointmentId}`, appointment)
            .then(() => {
                props.eventUpdated();
            });
    };
    ...
    const cancelAppointment = () => {
        axios_instance()
            .post(`/appointments/${props.appointmentId}/missed`)
            .then(() => {
            ...
            {!showDelete && (
                <>
                    <ChargeClientModal
                        ...
                        cancelFunction={() => {
                            setIsChargeClientModalOpen(false);
                        }}
                        isOpen={isChargeClientModalOpen}
                    ></ChargeClientModal>
                    <Dialog.Root open={props.isOpen}>
                        <Dialog.Content style={{ maxWidth: 450 }}>
                            <Flex justify="between">
                                <Dialog.Title className="text-slate-800">
                            ...
                            >
                                <Dialog.Close>
                                    ...
                                    >
                                        {t("appointment.make_missed")}
                                    </Button>
                                </Dialog.Close>
                                <Button
                                    disabled={appointmentPaid}
                                    className="w-full"
                                    onClick={() => {
                                        chargeClient();
                                    }}
