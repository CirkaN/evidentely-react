...
const CreateGymMembershipModal = (props: CreateGymMembershipProps) => {
    ...
    useQuery({
        queryFn: () => {
            axios_instance()
                .get("/gym_membership_plans")
                .then((r) => setAvailableMembershipPlans(r.data));
        },
    ...
    const [availableMembershipPlans, setAvailableMembershipPlans] =
        useState<GymMembershipPlanDTO[]>();
    const [transformedPlans, setTransformedPlans] =
        useState<TransformedDataForSelect[]>();

    const [clientList, setClientList] = useState<ClientDTO[]>();
    const [transformedClients, setTransformedClients] =
        useState<TransformedDataForSelect[]>();
    ...
    const setMembershipPlan = (
        e: SingleValue<{ value: string | number; label: string }>,
    ) => {
        if (e && e.value) {
            setForm((c) => c && { ...c, gym_membership_plan_id: e.value });

            const gymPlan = availableMembershipPlans?.find(
                (plan) => plan.id == e.value,
            );
            if (gymPlan?.duration_months) {
                const nextMonthDates = new Date(
                    new Date().setMonth(
                        new Date().getMonth() + gymPlan?.duration_months
                            ? gymPlan?.duration_months
                            : 1,
                    ),
                )
                    .toISOString()
        ...
        <>
            <Dialog.Root open={props.isOpen}>
                <Dialog.Content style={{ maxWidth: 450 }}>
                    <Dialog.Title>
                        {t("gym_memberships.create_membership")}
                    </Dialog.Title>
                    <form onSubmit={handleSubmit}>
                        <Flex direction="column" gap="3">
                            <label>
                                <Text as="div" size="2" mb="1" weight="bold">
                                    {t("gym_memberships.plan")}{" "}
