...
const ClientDetailsHeader = (props: DetailsProps) => {
    ...
    return (
        <div className="flex flex-col justify-between sm:flex-row border p-4 shadow-lg">
            ...
            <Link className={clsx("rounded-md py-2 px-4", location.pathname.includes("sms_history") && "text-blue-600 bg-blue-100 hover:bg-blue-200")} to={`/clients/${props.id}/sms_history`}>{t('common.sms_history')}</Link>
            <Link className={clsx("rounded-md py-2 px-4", location.pathname.includes("notes") && "text-blue-600 bg-blue-100 hover:bg-blue-200")} to={`/clients/${props.id}/notes`}>{t('common.notes')}</Link>
        </div>
