...
const SmsSettings = () => {
    ...
    const [phoneVerificationInProgress, setPhoneVerificationInProgress] =
        useState(false);
    ...
    const updateSmsSettings = (smsTemplate: SmsTemplate) => {
        closeModal();
        axios_instance()
            .put("/company/update_sms_templates", smsTemplate)
            .then(() => {
    ...
    const verifyCode = () => {
        axios_instance()
            .post("/company/attempt_verification", { code: codeReceived })
            .then((response) => {
    ...
    const attemptVerification = () => {
        axios_instance()
            .post("/company/verify_phone", phoneVerificationForm)
            .then((response) => {
    ...
    return (
        <>
            <EditSmsSettingsModal
                text={editModalText}
                saveFunction={(smsTemplate) => {
                    updateSmsSettings(smsTemplate);
                }}
                cancelFunction={() => {
                    closeModal();
                }}
                    ...
                    {!phoneVerificationInProgress && (
                        <>
                            <InfoBox
                                ...
                                headerText="Sms podesavanja"
                                text="Molimo verifikujte vas telefon da bi nastavili sa podesavanjima"
                            ></InfoBox>
                    ...
                    {phoneVerificationInProgress && (
                        <>
                            <InfoBox
                                type={InfoBoxType.Warning}
                                headerText="Sms podesavanja"
                                text="Molimo unesite kod koji ste dobili na vasem telefonskom uredjaju"
                            ></InfoBox>
                            <form onSubmit={handleVerifySubmit}>
                                {wrongCode && (
                                    <p className=" pt-5 text-center text-sm text-red-600">
                                        {t("sms.verification_failed")}
                                    </p>
                                )}
                                <div className="flex justify-center">
                                    <div className="w-1/3 text-center">
                                        ...
                                        <input
                                            type="text"
                                            onChange={(e) => {
                                                setCodeReceived(e.target.value);
                                            }}
                                            name="code"
                                            id="code"
                                        ...
                                        />
                                        <div className="pt-2">
                                            <button className="text-md rounded-md hover:bg-slate-400 bg-slate-300 p-2">
                                                {t("common.confirm")}
                                            </button>
                                        </div>
                                    </div>
            ...
            {userHasMobileVerified && (
                <>
                    <InfoBox
                        ...
                        text={t("sms.sms_char_warning")}
                    ></InfoBox>
                    <br />

                    <div className="px-4">
                        <p className="text-2xl text-center mb-5">
                            {t("sms.notify_clients_about_appointments")}
                        </p>
                        <div className="flex flex-col md:flex-row">
                            <SmsSettingsBox
                                onClickFunction={() => {
                                    openModal(
                                        smsSettings.sms_templates
                                            .reservation_confirmed,
                                        "reservation_confirmed",
                                    );
                                }}
                                headerText={t(
                                    "sms.client_reservation_confirmation",
                                )}
                                type="reservation_confirmed"
                                subHeaderText="Pri kreiranju termina"
                                mainText={
                                    smsSettings.sms_templates
                                        .reservation_confirmed
                                }
                            ...
                            <SmsSettingsBox
                                onClickFunction={() => {
                                    openModal(
                                        smsSettings.sms_templates.day_before,
                                        "day_before",
                                    );
                                }}
                                headerText={t("sms.day_before")}
                                type="day_before"
                                subHeaderText="u 19:00h"
                                mainText={smsSettings.sms_templates.day_before}
                            ></SmsSettingsBox>
                            <SmsSettingsBox
                                onClickFunction={() => {
                                    openModal(
                                        smsSettings.sms_templates.same_day,
                                        "same_day",
                                    );
                                }}
                                headerText={t("sms.same_day")}
                                type="same_day"
                                subHeaderText="2h pre termina"
                                mainText={smsSettings.sms_templates.same_day}
                            ></SmsSettingsBox>
                        </div>
                    </div>
                    ...
                    <div className="px-4">
                        ...
                        <div className="flex flex-col md:flex-row">
                            <SmsSettingsBox
                                ...
                                headerText={t("sms.confirmation_to_employee")}
                                type="employee_reservation_reminder"
                                subHeaderText="2 sata pre termina"
                                mainText={
                                    smsSettings.sms_templates
                                        .employee_reservation_reminder
                                }
                            ...
                            <SmsSettingsBox
                                ...
                                headerText={t("sms.confirmation_to_employee")}
                                type="employee_reservation_created"
                                subHeaderText="pri kreiranju termina"
                                mainText={
                                    smsSettings.sms_templates
                                        .employee_reservation_created
                            ...
                            <SmsSettingsBox
                                ...
                                headerText={t("sms.confirmation_to_employee")}
                                type="employee_reservation_changed"
                                subHeaderText="pri promeni termina"
                                mainText={
                                    smsSettings.sms_templates
                                        .employee_reservation_changed
                                }
