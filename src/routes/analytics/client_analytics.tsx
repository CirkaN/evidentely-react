...
const Analytics = () => {
    ...
    const [topCompletedClients, setTopCompletedClients] = useState<
        TopCompleted[]
    >([]);
    ...
    const fetchData = () => {
        ...
        if (calendar) {
            axios_instance()
                .post("/analytics/client_main", {
                    start_date: calendar[0],
                    end_date: calendar[1],
                })
                .then((r) => {
                    applyLiveData(r.data);
                });
        }
    };
                        ...
                        <>
                            <ResponsiveContainer width="100%" height="100%">
                                <BarChart
                                    width={150}
                                    height={40}
                                    data={topCompletedClients}
                                >
                                    <Bar
                                        dataKey="completed_appointments_count"
                                        name="Broj kompletiranih termina"
                                        fill="#8884d8"
                                    />
                                    <XAxis dataKey="name" />
                                    <YAxis />
                            ...
                            <p className="text-center">
                                Klijenti sa najvecim brojem termina
                            </p>
                        </>
                    )}
                    {topCompletedClients.length < 1 && (
                        <p className="text-center py-[150px]">
                            Trenutno nemamo podataka za prikaz, vremenom kada
                            unesete dovoljno podataka ovde ce se pojaviti
                            grafikon
                        </p>
                    )}
                </div>
                        ...
                        <>
                            <ResponsiveContainer width="100%" height="100%">
                                <BarChart
                                    width={150}
                                    height={40}
                                    data={topProfitClients}
                                >
                                    <Bar
                                        dataKey="total_amount_paid"
                                        name="Profit"
                                        fill="#5C8374"
                                    />
                                    <XAxis dataKey="name" />
                        ...
                        <>
                            <ResponsiveContainer width="100%" height="100%">
                                <BarChart
                                    width={150}
                                    height={40}
                                    data={topMissedClients}
                                >
                                    <Bar
                                        dataKey="missed_appointments_count"
                                        name="Broj propustenih termina"
                                        fill="#B31312"
                                    />
                                    <XAxis dataKey="name" />
                                    <YAxis />
                            ...
                            <p className="text-center">
                                Klijenti sa najvecim brojem propustenih termina
                            </p>
                        </>
                    )}
