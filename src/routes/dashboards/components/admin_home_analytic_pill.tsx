...
const AdminHomeAnalyticPill = (props: HomePillProps) => {
    ...
    const doFetch = () => {
        axios_instance()
            .post(`/analytics/pill`, filter)
            .then((r) => setResponseData(r.data));
    };
    const applyColors = () => {
    ...
    return (
        <>
            <div
                className={`max-w-sm w-full border  ${bgColor}  border-slate-300 rounded-lg  shadow-2xl  p-4 md:p-6 mt-2`}
            >
                <div className="flex justify-between">
                    <div>
                        <h5 className="leading-none text-3xl font-bold text-white pb-2">
                            ...
                            {filter.type !== "profit" && responseData.result}
                        </h5>
                ...
                <div className="grid grid-cols-1 items-center border-gray-200 border-t  justify-between">
                    <div className="flex justify-between items-center pt-5">
                        <p className="text-sm font-medium text-center text-white inline-flex items-center">
                            Poslednjih {filter.days} dana
                        </p>
                        <div className="w-48">
                            <select
                                value={filter.days}
